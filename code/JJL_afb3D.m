function [out] = JJL_afb3D(x, af1, af2, af3)
...
[L, H] = JJL_afb3D_A(x, af1, 1);

% filter along dimension 2
[LL LH] = JJL_afb3D_A(L, af2, 2);
[HL HH] = JJL_afb3D_A(H, af2, 2);

% filter along dimension 3
[LLL LLH] = JJL_afb3D_A(LL, af3, 3);
[LHL LHH] = JJL_afb3D_A(LH, af3, 3);
[HLL HLH] = JJL_afb3D_A(HL, af3, 3);
[HHL HHH] = JJL_afb3D_A(HH, af3, 3);
...
out = zeros(szx);
out(1:end/2,        1:end/2,        1:end/2) = LLL; 
out((end/2+1):end,  1:end/2,        1:end/2) = LLH;
out(1:end/2,        (end/2+1):end,  1:end/2) = LHL;
out((end/2+1):end,  (end/2+1):end,  1:end/2) = LHH;

out(1:end/2,        1:end/2,        (end/2+1):end) = HLL;
out((end/2+1):end,  1:end/2,        (end/2+1):end) = HLH;
out(1:end/2,        (end/2+1):end,  (end/2+1):end) = HHL;
out((end/2+1):end,  (end/2+1):end,  (end/2+1):end) = HHH;

% LOCAL FUNCTION

function [lo, hi] = JJL_afb3D_A(x, af, d)
